...
public class Application01 {
    ...
    public static void main(String[] args) {
        ...
        *   UnPooledDateSource : ConnectionPool 미사용 */
        ...
        /* index 4. DateBase 와 접근을 할 수 있는 클래스 등록 */
        configuration.addMapper(Mapper.class);

        /* index 3. DateBase 와 접속 할 정보를 만들었다. 이제 Session 구성 */
        /* comment.
        *   SqlSession 을 만들기 위한 준비 단계
        *   SqlSessionFactory : SqlSession 객체를 생성하기 위한 팩토리(공장)
        *                       역활의 인터페이스
        *   SqlSessionFactoryBuilder : SqlSessionFactory 인터페이스 타입의 하위 구현 객체를 생성하기 위한
        *                              빌드 역활을 수행한다.
        *   build() : 환경설정에 대한 정보를 담고 있는 Configuration 타입의 객체
        ...
        *   - boolean 타입의 인자로 전달 할 수 있다.
        *   - false 를 전달하게 되면
        *   - Connection 인터페이스 타입 객체로 DML 수행 후 autocommit
        *   - 설정을 꺼주게 된다. (권장) */
        SqlSession sqlSession = factory.openSession(false); // false 가 권장사항

        /* index 5. 등록한 Mapper 를 활용해서 내부에 작성한 기능을 수행 */
